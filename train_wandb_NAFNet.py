...
from utils.learning.train_part_wandb_NAFNet import train
...
def parse():
    ...
    parser.add_argument('--acc', type=int, default=[8, 9], help='accelerations on which the model will be trained')
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep_config, project="FIVarNet-n-att-ensemble-test")

    def train_using_wandb():
        # wandb run 하나 시작
        wandb.init(project = "FIVarNet-n-att-ensemble-test")
        args.net_name = Path('NAFNet'+str(wandb.config.cascade)+','+str(wandb.config.chans)+','+str(wandb.config.sens_chans)+','+str(wandb.config.unet_chans)+', acc '+str(args.acc[0])+' '+str(args.acc[1]))
