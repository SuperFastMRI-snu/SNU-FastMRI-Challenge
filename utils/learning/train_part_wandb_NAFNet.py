...
from utils.model.NAFNet.basicsr.models.archs.NAFNet_arch import NAFNet
...
def train_epoch(args, acc_steps, epoch, pretrained_model, refinement_model, data_loader, optimizer, LRscheduler, best_val_loss, loss_type):
    ...
    for iter, data in enumerate(data_loader):
        ...
        if iter < 4:
            axs[iter, 0].imshow(output.squeeze().detach().cpu().numpy(), cmap='gray')
            axs[iter, 1].imshow(output2.squeeze().detach().cpu().numpy(), cmap='gray')
        if iter == 4:
