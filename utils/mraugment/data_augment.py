...
class AugmentationPipeline:
    ...
    def augment_from_kspace(self, kspace, target_size, max_train_size=None):       
        ...
        target_augment_from_kspace = self.im_to_target(im, target_size)
        kspace = fft2c(im)
        
        return kspace, target_augment_from_kspace
    ...
    def im_to_target(self, im, target_size):     
        ...
        if len(im.shape) == 3: 
            # Single-coil
            target_im_to_target = complex_abs(T.complex_center_crop(im, cropped_size))
        else:
            # Multi-coil
            assert len(im.shape) == 4
            target_im_to_target = T.center_crop(rss_complex(im), cropped_size)
        return target_im_to_target  
