...
class TM_Att_FIVarNet_n_b(nn.Module):
    def __init__(
        self,
        num_cascades: int = 12,
        sens_chans: int = 8,
...
class TM_Att_FIVarNet_cs_n_b(nn.Module):
    def __init__(
        self,
        num_cascades: int = 12,
        sens_chans: int = 8,
        ...
        for ind in range(num_cascades):
            ...
            cascades.append(
                CSAttentionFeatureVarNetBlock(
                    ...
                    attention_layer=AttentionBlock(chans),
                    use_extra_feature_conv=use_image_conv,
                )
            )
...
class CSAttentionFeatureVarNetBlock(nn.Module):
    def __init__(
        self,
        encoder: FeatureEncoder,
        decoder: FeatureDecoder,
    ...
    def forward(self, feature_image: FeatureImage) -> FeatureImage:
        ...
        """
        feature_image = feature_image._replace(
            features=self.attention_layer(feature_image.features)
        )
        new_features = new_features - self.apply_model_with_crop(feature_image)
